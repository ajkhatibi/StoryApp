...
//
//  Created by Julian Khatibi on 6/16/16.
...
extension Story {
    ...
    var soundEffectURL: NSURL {
        let fileNmae: String
        switch self {
        case .Droid, .Home: fileNmae = "HappyEnding"
        case . Monster: fileNmae = "Ominous"
        default: fileNmae = "PageTurn"
        }
        let path = NSBundle.mainBundle().pathForResource(fileNmae, ofType: "wav")!
...
struct Adventure {
    static func story(name: String) -> Page {
        ...
        let homeWard = returnTrip.addChoice("Continue Home From Earth", story: .Homeward)
        let rover = touchDown.addChoice("Explore with Rover", story: .Rover(name))
